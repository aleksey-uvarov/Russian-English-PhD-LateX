...
We will not go deep into the theory of quantum channels, instead only highlighting a few interesting examples. For a more detailed account we refer the reader to Ref.~\cite{watrous_theory_2018}.
\begin{enumerate}
    \setcounter{enumi}{-1}
...
Let $V$ be the vector space of interest (most frequently we will deal with the qubit state space $\mathbb{C}^2$) and $V^*$ its dual space. A $(p, q)$\textit{-tensor} is a linear function from $V^q$ to $V^p$. The space of such tensors $T^p_q (V)$ is isomorphic to $V^* \otimes ... \otimes V^* \otimes V \otimes ... \otimes V$ with $q$ copies of $V^*$ and $p$ copies of $V$. In tensor network diagrams, such a tensor is depicted as a box with $p$ wires going right and $q$ wires going left.

\begin{equation}
    M^{i_1 ... i_p}_{j_1 ... j_q} = \vcenter{\hbox{\includegraphics{figures/inkscape/tensor}}}
\end{equation}
...
\begin{equation}
    A^i_j = \vcenter{\hbox{\includegraphics{figures/inkscape/box_a}}}
\end{equation}
We will draw a vector $v \in V$ as a triangle-shaped box with one wire sprouting to the left, while a covector $\delta \in V^*$ is depicted as a triangle-shaped box with one wire sprouting to the right. A projector $\ket{\psi} \bra{\psi}$ can then be depicted as follows:
\begin{equation}
    \ket{\psi} \bra{\psi} = \vcenter{\hbox{\includegraphics{figures/inkscape/path4585}}}
\end{equation}

The tensor product of (arbitrary) tensors is depicted by drawing the two tensors one above the other:
\begin{equation}
    A \otimes B = \vcenter{\hbox{\includegraphics{figures/inkscape/tensor_product}}}
\end{equation}
The contraction of tensors is depicted by connecting the corresponding wires:
\begin{equation}
    A \ket{v} = \vcenter{\hbox{\includegraphics{figures/inkscape/a_times_v}}}
\end{equation}
As long as the wires connect the correct inputs and outputs, the curves depicting the contractions of indices can be completely arbitrary. For example, taking the trace of a matrix requires connecting its input and output indices, which implies that we need to curve the wire into a loop (note that $\Tr A$ is a scalar, so the tensor network diagram has no loose wires):
\begin{equation}
    \Tr A = \vcenter{\hbox{\includegraphics{figures/inkscape/trace}}}
\end{equation}
An identity operator $\id: V \rightarrow V$ is depicted simply as a line segment:
\begin{equation}
    \id = \vcenter{\hbox{\includegraphics{figures/inkscape/line}}}
\end{equation}
If we bend this line segment one way or another, we can get a $(2, 0)$ or a $(0, 2)$ tensor. This corresponds to a tensor proportional to the maximally entangled state in $V \otimes V$ or its dual:
\begin{align}
    \sum_i \ket{e_i} \ket{e_i} &= \vcenter{\hbox{\includegraphics{figures/inkscape/bell}}} \\
    \sum_i \bra{e_i} \bra{e_i} &= \vcenter{\hbox{\includegraphics[angle=180]{figures/inkscape/bell}}}
\end{align}
Importantly, the when these diagrams occur on their own, they become basis dependent. Similarly, the transpose of a matrix is performed by turning both its wires in the opposite direction:
\begin{equation}
    A^T = \vcenter{\hbox{\includegraphics{figures/inkscape/transpose}}}
\end{equation}

% With a fixed choice of basis in $V$, there is an isomorphism of $V$ and $V^*$. 



% This allows us to change the signatures of tensors. Diagrammatically this amounts to bending the wires. We start with the identity operator that belongs to $V \otimes V^*$. In the basis $\{ \ket{e_i} \}$ it can be decomposed as $\id = \sum \ket{e_i} \bra{e_i}$. When we map $V^*$ to $V$, we replace the basis bra vectors with corresponding basis ket vectors, ending up with a tensor $\sum \ket{e_i} \ket{e_i} \in V \otimes V$, that is proportional to a maximally entangled state. Graphically we show this as follows:

% $\ni$

% In a similar manner, the transpose of a matrix is depicted by bending the incoming and outgoing wires by 180 degrees:

% PICTURE A TRANSPOSE

% EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE

% topologically, imagine boxes as vertices of a graph, and wires as edges; we construct a graph by taking a collection of unit segments $\left[0, 1\right]$ and identifying some endpoints. No that's not enough: every box has some order of outgoing wires.
...
% Since the authors develop the diagrammatic lagnuage for many other situations beside Hilbert spaces and tensors on these, it includes features we will not cover here. In the simplest formalization of tensor networks, one uses so-called ``progressive polarized graphs'', which are directed graphs with certain additional structure:
% \begin{enumerate}
%     \item A set of nodes of degree one called outer nodes (the rest are called inner nodes);
%     \item For every node $x$, the sets of incoming and outgoing edges $\operatorname{in}(x)$, $\operatorname{out}(x)$ are ordered;
% \end{enumerate}
% Two such graphs are considered isomorphic when they are isomorphic as graphs and the isomorphism preserves this extra structure. Finally, we associate a vector space with every edge and a linear map with every inner node. The domain and codomain of such linear maps are tensor products of spaces associated with $\operatorname{in}(x)$ and $\operatorname{out}(x)$. A graph together with this mapping is called a diagram. The article then shows that isomorphism of graphs implies that they define the same tensor.

% Formally, the authors use the idea of a ``generalized graph'', which is different from an ordinary graph in the sense that edges are only required to be one-dimensional manifolds. This means that they do not have to be connected to vertices, being loose on one or two ends instead. In addition, such a generalized graph can have circles as edges. Then this generalized diagram is given extra structure. All edges are oriented, and for every vertex, its sets of incoming and outgoing edges are ordered.
