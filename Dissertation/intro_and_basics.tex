...
\begin{example}
    Consider a quantum circuit consisting of two gates: a Hadamard gate acting on the first qubit and a CNOT gate acting on qubits $(1, 2)$. Starting from the state $\ket{00}$, this circuit prepares the Bell state $\frac{1}{\sqrt{2}}(\ket{00} + \ket{11})$. Before the application of the gates, the reduced density matrix of the first qubit was equal to $\ket{0} \bra{0}$, while after the gates, it is equal to $\frac12 \id$. These matrices have different rank, hence they cannot be conjugate to each other.
\end{example}
